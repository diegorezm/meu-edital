...
export default function ExamsScreen() {
  ...
  const filteredSubjects = subjects
    .filter((item) => normalize(item.name).includes(normalize(query.trim())))
  ...
  const removeExam = async (id: string) => {
    await update((current) => {
      const subjectIds = new Set(
        current.subjects
          .filter((item) => item.examId === id)
          .map((item) => item.id),
      );
      return {
        ...current,
        exams: current.exams.filter((item) => item.id !== id),
        subjects: current.subjects.filter((item) => item.examId !== id),
        topics: current.topics.filter(
          (item) => !subjectIds.has(item.subjectId),
        ),
        planned: current.planned.filter((item) => item.examId !== id),
        sessions: current.sessions.filter((item) => item.examId !== id),
        reviews: current.reviews.filter((item) => item.examId !== id),
        cycle: current.cycle.filter((item) => item.examId !== id),
        activeExamId:
          current.activeExamId === id
            ? current.exams.find((item) => item.id !== id)?.id || null
  ...
  const removeSubject = async () => {
    ...
    await update((current) => {
      ...
      return {
        ...
        topics: current.topics.filter((item) => item.subjectId !== id),
        planned: current.planned.filter((item) => item.subjectId !== id),
        sessions: current.sessions.filter((item) => item.subjectId !== id),
        cycle: current.cycle.filter((item) => item.subjectId !== id),
        reviews: current.reviews.filter((item) => !topicIds.has(item.topicId)),
      };
    });
  ...
  const removeTopic = async () => {
    ...
    await update((current) => {
      const ids = new Set([selectedTopic.id]);
      let changed = true;
      while (changed) {
        changed = false;
        for (const item of current.topics) {
      ...
      return {
        ...current,
        topics: current.topics.filter((item) => !ids.has(item.id)),
        planned: current.planned.filter((item) => !ids.has(item.topicId)),
        sessions: current.sessions.filter((item) => !ids.has(item.topicId)),
        reviews: current.reviews.filter((item) => !ids.has(item.topicId)),
        cycle: current.cycle.filter((item) => !ids.has(item.topicId)),
      };
    });
