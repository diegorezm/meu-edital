    ...
    data?.topics
      ...
      .map((item) => {
        const sessions = data.sessions.filter(
          (session) => session.topicId === item.id && session.questions,
        );
        const questions = sessions.reduce(
          (total, session) => total + (session.questions || 0),
          0,
        );
        const correct = sessions.reduce(
          (total, session) => total + (session.correct || 0),
          0,
        );
        return {
          topic: item,
          rate: questions ? Math.round((100 * correct) / questions) : null,
          count: data.sessions.filter((session) => session.topicId === item.id)
            .length,
        };
      }) || [];
