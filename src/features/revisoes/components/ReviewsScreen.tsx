...
export default function ReviewsScreen() {
  ...
  const reviews = data ? getOpenReviews(data) : [];
  const { due, later } = splitReviews(reviews, today());
  const topic = (id: string) =>
    data?.topics.find((item) => item.id === id)?.name || "Assunto";
  const complete = (id: string) =>
    update((current) => completeReview(current, id));
  const saveOffsets = async () => {
    const values = parseReviewOffsets(offsets);
    if (!values) {
      setError("Use dias inteiros, positivos e em ordem crescente.");
      return;
    }
    await update((current) => saveReviewOffsets(current, values));
    setOffsets("");
    setError("");
